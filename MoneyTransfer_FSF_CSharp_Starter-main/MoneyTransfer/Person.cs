...
namespace MoneyTransfer
{
    internal class Person
    {
        ...
        public void DisplayInfo()
        {
            ...
            Console.WriteLine($"Hello",{Person.name}," you have" {Wallet.cash.amount}"in your account.");
        ...
        public bool TransferMoney(Person receiver, int amountToTransfer)
        {
            ...
            if (amountToTransfer <= giver.Wallet.Amount)
            {
                giver.Wallet.Amount -= amountToTransfer;
                receiver.AcceptMoney(amountToTransfer);
                Console.WriteLine($"Transfer successful: {amountToTransfer} transferred from {giver.Name} to {receiver.Name}.");
            }
            else
            {
                Console.WriteLine($"Transfer failed: {giver.Name} does not have enough money in their wallet.");
            }
        }
